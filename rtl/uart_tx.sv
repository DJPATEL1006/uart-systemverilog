...
baud_gen #(.BAUD_DIV(BAUD_DIV)) u_baud (.clk(clk),.rst(rst),.baud_tick(baud_tick));
...
uart_fsm u_fsm(.clk(clk),.rst(rst),.baud_tick(baud_tick),.tx_start(tx_start),.bit_done(bit_done),.parity_bit(parity_bit),.serial_out(serial_bit),.tx(tx),.tx_busy(tx_busy),.load(load),.shift_en(shift_en),.count_en(count_en),.count_clear(count_clear));
endmodule